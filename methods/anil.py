...
class Learner(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.outer_optimizer = SGD(itertools.chain(self.meta_model.parameters(), self.classifier.parameters()), lr=self.outer_update_lr)
    ...
    def forward(self, batch_tasks, training = True, step = 0, epoch=0):
        ...
        for task_id, task in enumerate(batch_tasks):
            ...
            for i in range(0,num_inner_update_step):
                for inner_step, batch in enumerate(support_dataloader):
                    ...
                    loss = self.criterion(outputs.view(-1, self.num_labels), s_batch["labels"]) + self.reg_const * sum(
            ...
            if training:
                ...
                for i, params in enumerate(self.classifier.parameters()):
                    if task_id == 0:
                        sum_gradients_.append(params.grad)
                    else:
                        sum_gradients_[i] += params.grad
        ...
        if training:
            ...
            for i in range(0, len(sum_gradients_)):
                sum_gradients_[i] = sum_gradients_[i] / float(num_task)
